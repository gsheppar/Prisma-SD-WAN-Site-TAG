...
SCRIPT_NAME = 'CloudGenix: Example script: Site Tags'
SCRIPT_VERSION = "v1"
        ...
        try:
            ...
            for site in cgx.get.sites().cgx_content["items"]:
                if site["name"] == site_name:
                    ...
                    if site["tags"]:
                        if tags_list:
                            if collections.Counter(site["tags"]) == collections.Counter(tags_list):
                                print ("The site tags are arleady " + str(tags_list) + " on "  + site["name"]) 
                                update_tags = False
                    elif not tags_list:
                        print ("The site tags are arleady None on "  + site["name"]) 
                        update_tags = False            
                    if update_tags:
                        site["tags"] = tags_list
                        site_id = site["id"]
                        resp = cgx.put.sites(site_id=site_id, data=site)
                        if not resp:
                            print("Failed to update tags on " + site["name"] + " to " + str(tags_list))
                            print(str(jdout(resp)))
                        else:
                            print("Updated tags on " + site["name"] + " to " + str(tags_list))
        except:
            print("Failed to check tags on " + name["Site_Name"])
                    
  
    
    print("\nSite tags has been complete on all sites\n")
    return    
    ...
    try:
        with open(args['file'], "r") as csvfile:
            ...
            for row in csvreader:
                site_data = {}
                site_data["Site_Name"] = row['Site_Name']
                site_data["Tags"] = row['Tags']
                site_list.append(site_data)
    except:
        print("Error importing CSV. Please check column name Site_Name and Tags is there")
        return
    
    download(cgx, site_list)
